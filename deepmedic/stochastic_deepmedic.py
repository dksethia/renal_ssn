...
class StochasticDeepMedic(DeepMedic):
    def __init__(self,
        ...
        self.mean_l = nn.Conv2d(feature_maps[-1], num_classes, kernel_size=(1, ) * self.dim)
        self.log_cov_diag_l = nn.Conv2d(feature_maps[-1], num_classes, kernel_size=(1, ) * self.dim)
        self.cov_factor_l = nn.Conv2d(feature_maps[-1], num_classes * rank, kernel_size=(1, ) * self.dim)

    def forward(self, image, **kwargs):
        logits = F.relu(super().forward(image, **kwargs))
